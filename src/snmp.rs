...
mod tests {
    ...
    fn test_ber_unsigned_encoding_is_minimal_and_non_negative() {
        ...
        assert_eq!(encode_ber_unsigned(SNMP_TAG_COUNTER32, 127), vec![0x41, 1, 0x7f]);
        assert_eq!(
        ...
        assert_eq!(decode_ber_unsigned(&[0x80]), Err(SnmpError::InvalidBerEncoding));
    ...
    fn test_snmp_application_unsigned_values_use_required_sign_octet() {
        ...
        let raw = msg.try_serialize().unwrap();
        assert!(raw.windows(7).any(|w| w == [SNMP_TAG_COUNTER32, 5, 0, 0xff, 0xff, 0xff, 0xff]));

        msg.pdu.varbinds[0].value = SnmpValue::Counter64(u64::MAX);
        let raw = msg.try_serialize().unwrap();
        assert!(raw.windows(11).any(|w| w == [SNMP_TAG_COUNTER64, 9, 0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff]));
    }
