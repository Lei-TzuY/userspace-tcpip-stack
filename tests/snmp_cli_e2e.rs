...
impl Agent {
    ...
    fn wait_until_ready(&self) {
        ...
        let request = request.try_serialize().expect("serialize readiness request");
    ...
    fn client(&self, args: &[&str]) -> Output {
        Command::new(env!("CARGO_BIN_EXE_snmp_client"))
            .arg(&self.addr)
            .args(args)
            .output()
...
#[test]
fn client_get_set_get_round_trip_updates_agent_state() {
...
#[test]
fn read_only_set_is_rejected_without_mutating_state() {
    let agent = Agent::spawn();

    let denied = agent.client(&[
        "public",
        "set",
        SYS_NAME_OID,
        "string",
...
#[test]
fn set_reports_validation_errors_and_preserves_atomicity() {
    let agent = Agent::spawn();
