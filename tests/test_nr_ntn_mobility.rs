...
use toy_tcpip::nr_ntn_mobility::{
    ...
    NtnChoCandidate, NtnChoExecutionCondition, NtnMobilityEngine, SatelliteOrbitState,
    TargetSatellitePrecompensationServo, NTN_MOB_EARTH_RADIUS_M,
    NTN_MOB_SPEED_OF_LIGHT_M_S,
};

#[test]
fn test_ntn_satellite_orbit_and_elevation_tracking() {
    // 600 km LEO, equatorial orbit (inclination 0 deg), S-band 2.0 GHz
    let orbit = SatelliteOrbitState::new_leo(
        1,
        600_000.0,
        0.0,
        0.0,
    ...
    assert_eq!(orbit.semi_major_axis_m(), NTN_MOB_EARTH_RADIUS_M + 600_000.0);
    // Period for 600 km LEO is ~96.7 minutes = 5800 s
    let period = orbit.orbital_period_s();
    assert!(period > 5700.0 && period < 5900.0, "Period {period} s out of expected range");
...
fn test_earth_fixed_vs_earth_moving_beam_dynamics() {
    ...
    assert!(matches!(engine_fixed.beam_type, NtnBeamType::EarthFixed { .. }));
...
fn test_time_based_conditional_handover_trigger() {
    ...
    let mut engine = NtnMobilityEngine::new(
        ...
        serving_orbit,
        201,
        NtnBeamType::EarthFixed { cell_center_lat_deg: 0.0, cell_center_lon_deg: 0.0 },
    );
    ...
    engine.add_cho_candidate(NtnChoCandidate {
        ...
        target_sat_id: 2,
        target_cell_pci: 301,
        condition: NtnChoExecutionCondition::TimeBased { t1_threshold_s: 120.0 },
        target_orbit,
...
fn test_location_based_conditional_handover_trigger() {
    ...
    let mut engine = NtnMobilityEngine::new(
        ...
        serving_orbit,
        202,
        NtnBeamType::EarthMoving { beam_radius_km: 300.0 },
    );
...
fn test_target_satellite_ta_and_doppler_precompensation() {
    ...
    assert!(
        ...
        "Slant range {} m out of expected bounds", precomp.slant_range_m
    );

    // Elevation should be positive (visible above 10 deg mask, here ~22.2 deg)
    assert!(precomp.elevation_deg > 20.0, "Elevation {} deg too low", precomp.elevation_deg);
    ...
    assert!(
        ...
        "Doppler {} Hz exceeds satellite bounds", precomp.doppler_shift_hz
    );
}
...
fn test_feeder_link_switchover_lifecycle() {
    ...
    assert!(!fls.handle_packet(1400), "Packet must be buffered during muting");
    ...
    assert!(!fls.handle_packet(1400), "Packet must be buffered during rerouting");
    ...
    assert!(fls.handle_packet(1400), "Packet sent directly on target gateway");
...
fn test_end_to_end_ntn_mobility_engine_coordinator() {
    ...
    engine.add_cho_candidate(NtnChoCandidate {
        ...
        target_sat_id: 2,
        target_cell_pci: 200,
        condition: NtnChoExecutionCondition::ElevationThreshold { min_elevation_deg: 25.0 },
        target_orbit,
    ...
    assert!(el_220 < 25.0, "Elevation {el_220} deg should be below 25 deg");
