...
#[test]
fn failed_high_preference_router_stays_quarantined_over_unresolved_fallback() {
    let mut stack = stack();
    let fallback = ip("fe80::1");
    let preferred = ip("fe80::2");
    let prefix = ip("2001:db8:93::");
    let destination = ip("2001:db8:93::1234");

    stack.process_frame(&ra_frame_without_slla(
        fallback,
        prefix,
    ...
    let failed_at = NDP_DELAY_FIRST_PROBE_TIME_MS + 3 * NDP_RETRANS_TIMER_MS;
    let _ = stack.step_timers(failed_at);
    assert_eq!(stack.ndp_table.lookup(&preferred), None);
    assert_eq!(
        stack
            .ipv6_routing_table
