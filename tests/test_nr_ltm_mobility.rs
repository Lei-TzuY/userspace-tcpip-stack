...
use toy_tcpip::nr_ltm_mobility::{
    ...
    LtmCellSwitchCommandMacCe, LtmError, LtmMobilityEngine, LtmState, LtmSwitchMode,
    MAX_LTM_CANDIDATES,
};
...
fn test_ltm_l1_beam_measurements_and_best_candidate_tracking() {
    ...
    engine
        .update_l1_measurement(0, 1, -95.0, 12.0, 1000)
        .unwrap();
    engine
        .update_l1_measurement(1, 3, -82.0, 20.5, 1005)
        .unwrap();
    engine
        .update_l1_measurement(2, 4, -118.0, -5.0, 1010)
        .unwrap();
...
fn test_ltm_rachless_cell_switch_under_10ms() {
    ...
    let cand = LtmCandidateCell::new(0, 60, 3_500_000, 0x2222, vec![3, 7]).with_verified_ta(96);
    engine.add_candidate(cand).unwrap();

    engine
        .update_l1_measurement(0, 7, -78.0, 24.0, 500)
        .unwrap();
    ...
    assert_eq!(
        outcome.interruption_latency_ms,
        LTM_RACHLESS_SWITCH_LATENCY_MS
    );
    assert!(
        outcome.interruption_latency_ms < 10,
        "LTM RACH-less interruption must be < 10ms"
    );
...
fn test_ltm_cfra_fallback_when_ta_unaligned() {
    ...
    let cand = LtmCandidateCell::new(1, 70, 3_500_000, 0x3333, vec![1, 2]).with_cfra(18, 2);
    engine.add_candidate(cand).unwrap();
    engine
        .update_l1_measurement(1, 2, -84.0, 19.0, 600)
        .unwrap();
...
fn test_ltm_degraded_target_beam_triggers_fallback() {
    ...
    let cand = LtmCandidateCell::new(2, 80, 3_500_000, 0x4444, vec![5]).with_verified_ta(48);
    engine.add_candidate(cand).unwrap();

    // Target beam suffered sudden blockage (-122 dBm < -110 dBm minimum)
    engine
        .update_l1_measurement(2, 5, -122.0, -8.0, 700)
        .unwrap();
...
fn test_ltm_post_switch_failure_and_source_recovery() {
    ...
    let cand = LtmCandidateCell::new(0, 90, 3_500_000, 0x5555, vec![1]).with_verified_ta(32);
    engine.add_candidate(cand).unwrap();
    engine
        .update_l1_measurement(0, 1, -80.0, 20.0, 800)
        .unwrap();
