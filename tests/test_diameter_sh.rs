...
use toy_tcpip::diameter_sh::{
    HssShEngine, HssShSubscriberProfile, DATA_REF_REPOSITORY_DATA, DIAMETER_APPLICATION_SH,
...
fn test_diameter_sh_udr_and_snr() {
    ...
    assert_eq!(uda.get_avp(268).unwrap().as_u32().unwrap(), DIAMETER_SUCCESS);
    assert_eq!(
    ...
    assert_eq!(sna.header.command_code, DIAMETER_CMD_SUBSCRIBE_NOTIFICATIONS);
    assert_eq!(sna.get_avp(268).unwrap().as_u32().unwrap(), DIAMETER_SUCCESS);
