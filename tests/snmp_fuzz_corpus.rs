...
const MESSAGE_CORPUS_DIR: &str =
    concat!(env!("CARGO_MANIFEST_DIR"), "/fuzz/corpus/snmp_message_parse");
...
fn assert_panic_free_round_trip(seed: &[u8], label: &str) {
    ...
    if let Ok(message) = parsed {
        ...
        assert_eq!(reparsed, message, "round trip changed corpus entry: {label}");
    }
}
