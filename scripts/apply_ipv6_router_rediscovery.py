...
Path("tests/test_ipv6_router_rediscovery.rs").write_text(r'''use std::str::FromStr;
...
fn default_router_expiry_immediately_starts_rediscovery_and_keeps_address() {
    ...
    assert_eq!(frames.len(), 1, "router expiry must immediately solicit a replacement");
...
fn virtual_lab_router_expiry_rediscovery_restores_default_route_end_to_end() {
    ...
    assert_eq!(lab.host("host").unwrap().stack.ipv6_gateway(), Some(router_ll));
