...
src = Path("src/sflow.rs")
text = src.read_text()
old = '''        for _ in 0..samples_count {
            if offset + 8 > data.len() {
                break;
            }
'''
new = '''        for _ in 0..samples_count {
            if data.len() - offset < 8 {
                return Err(SflowError::InvalidLength);
            }
'''
if old not in text:
    raise SystemExit("sample header marker not found")
text = text.replace(old, new, 1)
old = '''            if offset + sample_len > data.len() {
                break;
            }
'''
new = '''            if sample_len > data.len() - offset {
                return Err(SflowError::InvalidLength);
            }
'''
if old not in text:
    raise SystemExit("sample body marker not found")
src.write_text(text.replace(old, new, 1))

Path("tests/test_sflow_sample_framing.rs").write_text('''use toy_tcpip::sflow::{SflowDatagram, SflowError};
