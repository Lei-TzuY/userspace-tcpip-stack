...
parser, and from `tests/sample-analysis.pcap`, which is the only fixture
carrying SACK options, duplicate-ACK runs, and zero-window advertisements — so
it is what reaches the expert analysis at all. The rest are hand-built
malformed inputs aimed at the places where the stack walks an
