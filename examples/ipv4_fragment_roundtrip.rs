...
fn parse_usize_arg(name: &str, default: usize) -> Result<usize, String> {
    ...
    match env::args().skip(1).find_map(|arg| arg.strip_prefix(&prefix).map(str::to_owned)) {
        Some(value) => value
            .parse::<usize>()
...
fn run() -> Result<(), String> {
    ...
    let payload = (0..payload_len).map(|i| (i % 251) as u8).collect::<Vec<_>>();
    ...
    for wire in fragments.iter().rev() {
        ...
        let packet = Ipv4Packet::parse(wire).map_err(|err| format!("fragment parse failed: {err}"))?;
